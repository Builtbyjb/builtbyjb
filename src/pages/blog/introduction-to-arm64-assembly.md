...
Assembly language instruction sets are different across CPU architectures. This blog focuses on the ARM64 instruction set for Apple Silicon, but the underline knowledge can be transferred to other ARM64 based devices as well.

> This blog post assumes you have some programming experience and a basic understanding computer programing concepts.
...
<!--TODO: talk about register breakdown and usage-->
Now that we have covered the basics of ARM64 registers, we move on to the instruction sets. Assembly code is read from top to bottom with each line containing an instruction to the CPU.
...
In most cases the destination register comes first, followed by the source register. In the **add** instruction x11 is the destination register and x9 and x10 are the source registers.
...
the **.align** directive is used to align the code to a specific boundary. In this case, it is aligned to a 4bytes boundary.

The `cmp    x0, #4` instruction checks if the length of the command line arguments is less than 4. If it is not, the program jumps to the **_invalid_argument** function.
...
The **bl** instruction tells the program to jump to the specified instruction and store the address of where it jumped from in the link register. This allows the program to return to the correct location after the function call.
...
_return_check_float:
    ret
```
The **_check_float_setup** setups our control variable by moving the 0 to the x12 register. The reason assembly functions are more like labels is because without a **ret** or a **b** instruction the flow execution moves to the next function. This makes the code structure very important.
...
_return_int:
    ...
    ret
```
In the **_str_to_int** function we store the result of the string to integer operation in the x13 register. Which is initially set to 0. The second instruction mov the immediate value 10 to the x11 register. The third instruction store moves the number zero to x12 register, the x12 registers keeps track of the sign of the number ( 0 = +, 1 = -). 
...
The **madd** updates the result in x13 by adding current value of x13 to constant in x14 and multiplying the result by x9 (Remember the x9 and w9 point to same physical memory. Even though they point to the same physical memory we can not perform some instructions between the different because of the difference in size).
...
After all the commandline values have been converted to integers. The next step is to compare the the operator which is stored in the w9 register. The **_compare** function compares the operators and branches to the right operation function when a match is found or calls the **_invalid_argument** function if no match is found.
```asm
...
We are dividing the value in the x11 register (x11 stores the result of the arithmetic operations) by the value in the w13 register and storing the result in the x11 register. We then call the **msub** instruction which multiples the value in the x14 register by the value in the x13 register and then subtracts the result from the value in x11 register. The result of the operation is stored in the x15 register. 
...
_n_copy:
   ...
   ret
```
The last two instructions of the **_str_convert_loop** stores 0 in the x14 register, and stores the last index of the values stored on the stack in x9 register.
...
The **_int_to_str** function already stores the values need in the right registers. The x2 register stores the count and the x1 registers stores the result of the conversion.
```asm 
_print_value:
    mov     x0, #0
...
In the **_division** function we check for "zero division errors" by comparing the denominator to 0, branches to the **_zero_division_error** function when the denominator is zero, and prints the the **zero_division_error_msg**. Following the same steps we took when printing the results of our arithmetic operations.
```asm
...
```
The length of the output message must be exact, because it tells the syscall when to stop reading values. A smaller length will cause the syscall to stop reading values before the end of the message, resulting in an incomplete message being displayed. A larger length will cause the syscall to read more values than necessary, resulting in garbage values being added to the output.

##### Floating Point Operations
Up to this point, our program only supports integer operations. To handle floating point operations, we need to convert the string input to floats instead of integers. This is a lot of complicated, to make our lives easier we will use the **atof** function in the **c** standard library. 
...
We start by refractor the main function to use the **atof** function.
```asm 
.global _main
.align 4
.text
...
```
The **.text** directive denotes the start of the text section of the program. This section contains the machine code instructions that will be executed by the CPU.

The first instruction stores the values in the x29 and x30 registers on the stack. The next instruction stores the value of the stack pointer in x29 register.

We have not been following the ARM64 calling convention, because we have been writing a lot of the function ourselves, external functions follow the ARM64 calling convention. One of these conventions in that the function arguments are stored in registers x0 - x8 with the remaining arguments stored on the stack. First function argument is x0 and so on.

To use the **atof** function we have to store the numbers in the x0 register. The **atof** stores the return value in d0 register (d registers are used for floating point operations). we then move the d11 register, the same is done for the second number with value being stored in the d12 register.
...
The **_exit** function program by storing 0 the w0 register and the values of the x29 and x30.
...
_zero_division_error:
    ...
    b       _exit
```
We can not just load the addresses of **fmt_result** and **fmt_str** variables the way we loaded **new_line** variable because variables stored in the data section are not directly accessible. Instead, we need to use the **adrp** and **add** instructions to load the address of the variable into a register.

##### Conclusion

The mathematical operations themselves where just a single instruction. A lot of the work was format conversion and printing to the commandline. Working with assembly language makes you appreciate the high level abstractions that are provided by higher level languages.
